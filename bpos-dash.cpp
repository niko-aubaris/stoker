...
struct Row {
    ...
    bool sky_est = false;                   // surplus figures are an estimate
...
static const char* STOKER_VERSION = "v2.7.4";
...
static void ingest(const std::string& raw) {
    ...
    for (auto& s : d.value("skyhooks", json::array())) try {
        ...
        r.sky_est = s.value("est", false);
        r.sky_last_raided = sstr("last_raided_at");
